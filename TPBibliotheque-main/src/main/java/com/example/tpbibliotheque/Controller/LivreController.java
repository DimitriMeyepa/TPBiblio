...
public class LivreController {
    ...
    @FXML private TextField titreField, auteurField, isbnField, anneeField, rechercheField;
    ...
    @FXML private Button ajouterButton, modifierButton, supprimerButton, annulerButton;
    ...
    public void initialize() {
        ...
        livreTable.setItems(filteredData);
        ...
        rechercheField.textProperty().addListener((obs, oldValue, newValue) -> {
            filteredData.setPredicate(livre -> {
                ...
                return livre.getTitre().toLowerCase().contains(lowerCaseFilter)
                        || livre.getAuteur().toLowerCase().contains(lowerCaseFilter)
                        || livre.getCodeISBN().toLowerCase().contains(lowerCaseFilter);
            });
        });
    }
    ...
    public void ajouterLivre() throws SQLException {
        ...
        int annee;
        try { annee = Integer.parseInt(anneeText); } catch (NumberFormatException e) {
    ...
    private void refresh() {
        if (dao == null) return;
        try { data.setAll(dao.readAll()); } catch (SQLException e) { e.printStackTrace(); }
    }
